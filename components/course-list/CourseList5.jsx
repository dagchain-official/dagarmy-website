...
export default function CourseList5() {
  ...
  const [sorted, setSorted] = useState(dagarmyCourses);
  ...
  useEffect(() => {
    ...
    if (selectedCategories.length) {
      const filteredByCategories = [...dagarmyCourses].filter((elm) =>
    ...
    if (rating.length) {
      const filteredByRating = [...dagarmyCourses].filter((elm) =>
    ...
    if (selectedPrices.length) {
      ...
      if (selectedPrices.includes("Paid")) {
        filteredByPaid = [...dagarmyCourses].filter((elm) => elm.price != 0);
      }
      if (selectedPrices.includes("Free")) {
        filteredByFree = [...dagarmyCourses].filter((elm) => elm.price == 0);
      }
      const filterByPrice = [...filteredByPaid, ...filteredByFree];
      filteredArrays = [...filteredArrays, filterByPrice];
    }
    const commonItems = [...dagarmyCourses].filter((item) =>
