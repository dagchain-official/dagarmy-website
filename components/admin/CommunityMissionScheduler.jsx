...
const PLATFORMS = ['youtube','instagram','twitter','tiktok','facebook','linkedin','discord','telegram','coinmarketcap'];
const ACTIONS = [
  { value: 'subscribe', label: 'Subscribe',      points: 150 },
  { value: 'follow',    label: 'Follow',          points: 150 },
  { value: 'like',      label: 'Like',            points: 50  },
  { value: 'comment',   label: 'Comment',         points: 75  },
  { value: 'share',     label: 'Share / Repost',  points: 75  },
  { value: 'retweet',   label: 'Retweet',         points: 50  },
  { value: 'watch',     label: 'Watch Video',     points: 50  },
  { value: 'join',      label: 'Join Group/Server',points: 150 },
  { value: 'tag',       label: 'Tag a Friend',    points: 50  },
];
...
export default function CommunityMissionScheduler() {
  ...
  const downloadSample = () => {
    const sample = [
      CSV_HEADERS.join(','),
      '2026-04-01,youtube,subscribe,Subscribe to our YouTube Channel,"Subscribe to the DAGChain YouTube channel and stay updated",https://youtube.com/@DAGChain,150,1',
      '2026-04-02,twitter,follow,Follow us on Twitter/X,"Follow @DAGChain_Official on Twitter/X",https://twitter.com/DAGChain_Official,150,1',
      '2026-04-03,instagram,like,Like our Latest Instagram Post,"Like the pinned post on our Instagram page",https://instagram.com/p/EXAMPLE,50,1',
      '2026-04-04,youtube,comment,Comment on our Latest Video,"Leave a thoughtful comment on our latest YouTube video",https://youtube.com/watch?v=EXAMPLE,75,1',
      '2026-04-05,telegram,join,Join our Telegram Community,"Join the official DAGChain Telegram group",https://t.me/DAGChain,150,1',
    ].join('\n');
    const blob = new Blob([sample], { type: 'text/csv' });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = url; a.download = 'community_missions_template.csv'; a.click();
  ...
  const handleFile = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setRows(parsed);
        setGlobalMsg({ type: 'success', text: `Loaded ${parsed.length} community missions from file. Review and publish below.` });
      } catch (err) {
  ...
  const publishAll = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setGlobalMsg({ type: 'success', text: `${data.created} community mission${data.created !== 1 ? 's' : ''} scheduled successfully!` });
      } else {
    ...
    <div style={{ padding: '0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <button onClick={downloadSample}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '10px', border: '1.5px solid rgba(99,102,241,0.3)', background: '#eef2ff', color: '#6366f1', fontSize: '12px', fontWeight: '700', cursor: 'pointer' }}>
            <Download size={14} /> Download Template
          </button>
          <button onClick={() => fileRef.current?.click()}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '10px', border: '1.5px solid rgba(0,0,0,0.08)', background: '#fff', color: '#0f172a', fontSize: '12px', fontWeight: '700', cursor: 'pointer' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '1000px' }}>
            ...
            <tr style={{ background: '#f8fafc', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
              {[
                ...
                <th key={i} style={{ padding: '10px 12px', fontSize: '10px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.5px', textAlign: 'left', width: col.w }}>
                  {col.label}
                  {col.note && <div style={{ fontSize: '9px', fontWeight: '600', color: '#6366f1', textTransform: 'none', letterSpacing: 0, marginTop: '1px', opacity: 0.7 }}>{col.note}</div>}
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((row, idx) => {
              ...
              return (
                <tr key={row._id} style={{ borderBottom: '1px solid rgba(0,0,0,0.04)', background: hasError ? '#fff5f5' : isSaved ? '#f0fdf4' : idx % 2 === 0 ? '#fff' : '#fafbfc' }}>
                  ...
                  <td style={{ padding: '8px 10px' }}>
                    <input value={row.target_url} onChange={e => updateRow(row._id, 'target_url', e.target.value)}
                      ...
                      style={{ ...inputStyle,
                        borderColor: row._error?.includes('URL') ? '#fca5a5' : row.target_url ? 'rgba(99,102,241,0.3)' : 'rgba(0,0,0,0.08)',
                        background: row.target_url ? '#fafbff' : '#fff'
                      }} disabled={isSaved} />
                  </td>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '16px', flexWrap: 'wrap', gap: '10px' }}>
        <button onClick={addRow}
          style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '9px 18px', borderRadius: '10px', border: '1.5px dashed rgba(0,0,0,0.12)', background: '#fff', color: '#64748b', fontSize: '12px', fontWeight: '700', cursor: 'pointer' }}
