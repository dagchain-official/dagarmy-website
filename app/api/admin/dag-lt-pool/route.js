  ...
  try {
    ...
    if (action === 'add_member') {
      ...
      const { error: upsertErr } = await supabase
        .from('dag_lt_pool_members')
        .upsert({ user_id: user.id, is_active: true, enrolled_at: new Date().toISOString(), qualified_at: new Date().toISOString(), notes: notes || 'Manually enrolled by admin' }, { onConflict: 'user_id' });
      if (upsertErr) return NextResponse.json({ error: 'Failed to enroll member', details: upsertErr.message }, { status: 500 });
      return NextResponse.json({ success: true, message: `${user.full_name || user.email} enrolled in DAG LT Pool` });
    }
