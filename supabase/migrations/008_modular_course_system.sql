...
CREATE TABLE IF NOT EXISTS public.lessons (
    ...
    lesson_number TEXT NOT NULL, -- e.g., "1.1", "1.2"
    title TEXT NOT NULL,
    type TEXT NOT NULL, -- theory, drill, strategy, graduation
    description TEXT,
    duration TEXT,
    content TEXT, -- Rich text content for the lesson
    video_url TEXT,
    resources JSONB, -- Additional resources, links, files
    order_index INTEGER NOT NULL,
    is_published BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE INDEX IF NOT EXISTS idx_user_enrollments_user_id ON public.user_program_enrollments(user_id);
CREATE INDEX IF NOT EXISTS idx_user_enrollments_program_id ON public.user_program_enrollments(program_id);
...
DROP TRIGGER IF EXISTS trigger_update_program_progress ON public.user_module_progress;
