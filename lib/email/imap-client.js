...
export async function getMailboxFolders(accountEmail) {
  const cached = await getCachedFolders(accountEmail);
  ...
  try {
    ...
    await setCachedFolders(accountEmail, folders);
    return folders;
  } finally {
    await client.logout();
  }
...
export async function fetchMessages(accountEmail, folder = 'INBOX', { page = 1, limit = 25 } = {}) {
  ...
  try {
    ...
    await setCachedList(accountEmail, folder, page, limit, result);
    return result;
  } finally {
    await client.logout();
  }
...
export async function fetchMessage(accountEmail, folder = 'INBOX', uid) {
  const cached = await getCachedMessage(accountEmail, folder, uid);
  if (cached) return cached;
  ...
  try {
    ...
    if (result) await setCachedMessage(accountEmail, folder, uid, result);
    return result;
  } finally {
    await client.logout();
  }
...
export async function setMessageSeen(accountEmail, folder, uid, seen = true) {
  await invalidateList(accountEmail, folder);
  await invalidateMessage(accountEmail, folder, uid);
  const client = createClient(accountEmail);
  await client.connect();
  try {
...
export async function moveMessage(accountEmail, sourceFolder, uid, targetFolder) {
  await invalidateList(accountEmail, sourceFolder);
  await invalidateList(accountEmail, targetFolder);
  await invalidateMessage(accountEmail, sourceFolder, uid);
  const client = createClient(accountEmail);
  await client.connect();
  try {
